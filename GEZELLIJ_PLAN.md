...
- [x] Live upgrade, exec-in-place variant (chosen over the socket handover for the MVP: same pid, children stay waitable, systemd MainPID unchanged, no cross-process protocol). `SIGUSR2` → the server snapshots its layout, records pane↔PTY correlation in `<sock dir>/handover/<session>.exec.json`, marks everything but the PTY masters close-on-exec and `execve`s the new binary with `--server <sock> --adopt <manifest>`; the successor rebuilds the session from the layout and adopts the live PTYs leaf by leaf (`Pty::try_adopt_pane`, `ServerOsApi::adopt_terminal`). *(verified: `sleep 100000` and a supervised `tick` service survive with unchanged pids; the tick counter kept counting)*
...
- [ ] Hardening: tell attached clients to re-attach instead of dropping them; auto-offer the upgrade on `attach` when the binary is detected as replaced; layout-compat corpus test.
